...
from my_layers import Attention, Average, WeightedSum, WeightedTopicEmb, MaxMargin
...
def create_model(args, maxlen, vocab):
    ...
    ##### Reconstruction #####
    p_t = Dense(args.num_topics)(z_s)
    p_t = Activation('softmax', name='p_t')(p_t)
    r_s = WeightedTopicEmb(args.num_topics, args.emb_dim, name='topic_emb',
    ...
    ### Word embedding and topic embedding initialization ######
    if args.emb_path:
        from embReader import EmbReader as EmbReader
        emb_reader = EmbReader(args.emb_path, emb_dim=args.emb_dim, wv_type=args.emb_type)
        ...
        logger.info('Initializing topic embedding matrix as centroid of kmean clusters')
        K.set_value(model.get_layer('topic_emb').W, emb_reader.get_topic_matrix(args.num_topics, args.fix_clusters))
